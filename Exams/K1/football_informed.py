...
class Football(Problem):
    ...
    def h(self, node):
        x = node.state[1][0]
        y = node.state[1][1]
        x1 = self.goal_pos[0][0]
        y1 = self.goal_pos[0][1]
        x2 = self.goal_pos[1][0]
        y2 = self.goal_pos[1][1]
        return min((abs(x - x1) + abs(y - y1))/2, (abs(x - x1) + abs(y - y1))/2)
