...
class GhostOnSkates(Problem):
    def __init__(self, initial, walls, n_n, goal=None):
    ...
    def check_valid(state, walls, size):
        ...
        if duhche in walls:
            return False
        if not (0 <= duhche[0] < size and 0 <= duhche[1] < size):
            return False
        return True
    ...
    def successor(self, state):
        ...
        # gore 1
        new_position = (duhche[0], duhche[1] + 1)
        if self.check_valid(new_position, obstacles, grid_size):
            successors["Gore 1"] = new_position
        # gore 2
        new_position = (duhche[0], duhche[1] + 2)
        if self.check_valid(new_position, obstacles, grid_size):
            successors["Gore 2"] = new_position
        # gore 3
        new_position = (duhche[0], duhche[1] + 3)
        if self.check_valid(new_position, obstacles, grid_size):
            successors["Gore 3"] = new_position
        # desno 1
        new_position = (duhche[0] + 1, duhche[1])
        if self.check_valid(new_position, obstacles, grid_size):
            successors["Desno 1"] = new_position
        # desno 2
        new_position = (duhche[0] + 2, duhche[1])
        if self.check_valid(new_position, obstacles, grid_size):
            successors["Desno 2"] = new_position
        # desno 3
        new_position = (duhche[0] + 3, duhche[1])
        if self.check_valid(new_position, obstacles, grid_size):
            successors["Desno 3"] = new_position
