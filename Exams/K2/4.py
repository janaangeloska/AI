...
if __name__ == '__main__':
    otstrani_kolona = int(input())
    broj_drva = int(input())
    kriterija = input()
    classifier = RandomForestClassifier(n_estimators=broj_drva,criterion=kriterija, random_state=0)

    #otstrani ja kolonata so indeks
    if otstrani_kolona == 0:
        new_dataset = [row[1:] for row in dataset]
    else:
        new_dataset = [row[:otstrani_kolona] + row[otstrani_kolona+1:] for row in dataset]
    ...
    accr = accuracy_score(test_y, predicts)
    ...
    if otstrani_kolona == 0:
        ...
        new_sample = new_sample[:otstrani_kolona] + new_sample[otstrani_kolona+1:]

    new_prediction = classifier.predict([new_sample])[0]
    new_prediction_probabilities = classifier.predict_proba([new_sample])[0]

    print(new_prediction)
    print(new_prediction_probabilities)
