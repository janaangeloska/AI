...
if __name__ == '__main__':
    # 4 modeli na klasifikacija
    baesov = GaussianNB()
    drvo_na_odluka = DecisionTreeClassifier(criterion='entropy', random_state=0)
    kolekcija_drva = RandomForestClassifier(10, criterion='entropy', random_state=0)
    nevronska = MLPClassifier(10, activation='relu', learning_rate_init=0.001, random_state=0)

    procenti = int(input()) / 100.0
    nuli = [x for x in dataset if x[-1] == 0]
    edinici = [x for x in dataset if x[-1] == 1]

    train_set = nuli[:int(procenti * len(nuli))] + edinici[:int(procenti * len(edinici))]
    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = nuli[int(procenti * len(nuli)):] + edinici[int(procenti * len(edinici)):]
    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    baesov.fit(train_x, train_y)
    drvo_na_odluka.fit(train_x, train_y)
    kolekcija_drva.fit(train_x, train_y)
    nevronska.fit(train_x, train_y)

    acc_baes = baesov.score(test_x, test_y)
    acc_drvo = drvo_na_odluka.score(test_x, test_y)
    acc_shuma = kolekcija_drva.score(test_x, test_y)
    acc_nevronska = nevronska.score(test_x, test_y)

    accuracies = {
        'Naive Bayes': acc_baes,
        'Decision Tree': acc_drvo,
        'Random Forest': acc_shuma,
        'MLP': acc_nevronska
    }

    bestClassifier = max(accuracies, key=accuracies.get)
    print(f'Najgolema tocnost ima klasifikatorot {bestClassifier}')

    pred_b = baesov.predict(test_x)
    pred_d = drvo_na_odluka.predict(test_x)
    pred_s = kolekcija_drva.predict(test_x)
    pred_n = nevronska.predict(test_x)

    weights = {
        name: 2 if name == bestClassifier else 1 for name in accuracies
    }
    ...
    for i in range(len(test_x)):
        ...
        votes[pred_b[i]] += weights['Naive Bayes']
        votes[pred_d[i]] += weights['Decision Tree']
        votes[pred_s[i]] += weights['Random Forest']
        votes[pred_n[i]] += weights['MLP']
        predictions.append(0 if votes[0]>votes[1] else 1)

    TP = sum(1 for i in range(len(test_y)) if test_y[i] == 1 and predictions[i] == 1)
    FN = sum(1 for i in range(len(test_y)) if test_y[i] == 1 and predictions[i] == 0)
